...
sample. \texttt{distance.h} is MPI-incompatible, so this polyline is
converted to a restart dump in serial and then reused.

The repository ships \texttt{Bo0.0010.dat} as the current default
Stage~1 cavity. Regenerating that Bond number from a cold start places
the south pole within $0.01\,\Rzero$ of the shipped file, keeps the
volume residual below $10^{-6}$, and recovers the small-Bond opening
\eqref{eq:alpha} to better than eight percent. A cold start at
$\Bo=1$ walks the continuation ladder and does not return to the axis
after leaving the south pole.
